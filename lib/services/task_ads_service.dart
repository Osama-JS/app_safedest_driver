...
class TaskAdsService {
  ...
  /// Reject a task after offer is approved by customer
  Future<ApiResponse<Map<String, dynamic>>> rejectTask(int offerId) async {
    try {
      debugPrint('Rejecting task for offer ID: $offerId');
      ...
      debugPrint('Reject task response: ${response.success}');
      return response;
    } catch (e) {
      debugPrint('Error rejecting task: $e');
      return ApiResponse<Map<String, dynamic>>(
        success: false,
        message: 'Failed to reject offer: $e',
      );
    }
  }
  ...
  bool canAcceptTask(TaskAd ad) {
    // Driver can accept if their offer is accepted and ad is not yet closed (or just accepted status)
    // Backend acceptTask closes the ad, so we should allow it if it's NOT closed or if it's running with accepted offer.
    // Ideally: myOffer is accepted, and I haven't clicked accept yet (which closes the ad).
